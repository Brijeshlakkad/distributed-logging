...
func (l *DistributedLog) setupRaft(dataDir string) error {
	...
	// Log store where Raft stores those commands.
	...
	// Stable store where Raft stores the cluster’s configuration.
	...
	// Snapshot store where Raft stores compact snapshots of its data.
	retain := 1
	...
	if l.config.Raft.Bootstrap && !hasState {
		config := raft.Configuration{
			Servers: []raft.Server{{
				ID:      config.LocalID,
				Address: raft.ServerAddress(l.config.Raft.BindAdr),
			}},
		}
...
// Invokes this method after committing a log entry.
